...
void update(int index,int start,int end,int x,int val){
	if(start>end) return;
	if(start==end){tree[index].best=tree[index].sum=tree[index].sufmax=tree[index].premax=val;return;}//leaf index
	int mid=(start+end)/2;
	if(x<=mid)
		update(2*index,start,mid,x,val);
	else
     	update(2*index+1,mid+1,end,x,val);
    //tree[index]=node();
    tree[index].merge(tree[2*index],tree[2*index+1]);
}
int main(){
